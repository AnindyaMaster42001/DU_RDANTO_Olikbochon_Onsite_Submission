"""Honest OOF gate for the gemma27 cross-lingual signal.

Reproduces the exact 11-signal 0.828 stack (base8 + j32sv + ret32 + sa) from
results/, then measures what adding signal_gemma27 does. Reports macro-F1 (public
LB proxy) AND class-0/hallucinated F1 (the real Phase-2 metric), split
overall/ctx/noctx, plus a focused readout on the gemma27-ACTIVE subset (rows the
signal actually grounded, != 0.5) and net rescues vs the baseline stack.

Gate (prior discipline, 299 rows): +gemma27 must lift OOF beyond ~0.02 noise
(esp. noctx / factual-GK bucket) OR produce net rescues on the GK wall.

Usage:  python3 validate_gemma27.py
Reads signals from results/ (canonical) and signal_gemma27.json from cwd/results.
"""
...
HAVE_ENWIKI = "gemma27" in SIGS
print(f"loaded {len(SIGS)} signals; gemma27 present: {HAVE_ENWIKI}\n")
...
def main():
    ...
    for metric, mname in [(C.macro_f1, "macroF1"), (class0_f1, "F1-hall")]:
        ...
        if HAVE_ENWIKI:
            enw_hard, enw_soft = report("+gemma27", STACK + ["gemma27"], metric, mname)
        print()

    if not HAVE_ENWIKI:
        print("signal_gemma27.json not found yet — run after the kernel completes.")
        return

    # --- focused analysis on the subset gemma27 actually grounded (!= 0.5) ---
    ev = np.asarray(SIGS["gemma27"]["samples"], float)
    active = (np.abs(ev - 0.5) > 1e-6) & (~CTX_S)
    ai = np.arange(len(Y))[active]
    print("=" * 72)
    print(f"gemma27-ACTIVE labeled subset: {len(ai)} rows "
          f"({int((Y[ai]==0).sum())} hall / {int((Y[ai]==1).sum())} faithful)")
    if len(ai):
        stand = (ev[ai] >= 0.5).astype(int)
        acc = (stand == Y[ai]).mean()
        print(f"  gemma27 standalone acc on active subset: {acc:.3f}")

    # net rescues: rows the base stack got wrong that +gemma27 fixes (and vice-versa)
    base_hard, _ = report("base (11-sig)", STACK, C.macro_f1, "macroF1")
    enw_hard, _ = report("+gemma27", STACK + ["gemma27"], C.macro_f1, "macroF1")
    ...
    print(f"\nrows changed by gemma27: {len(changed)}   fixed {len(fixed)}   broke {len(broke)}"
